...
public class BasicStreamsExercise02 {
    ...
    public List<String> getProductNamesForCategory(List<Product> products, Category category) {
        ...
        return products.stream().filter(product -> product.getCategory().equals(category)).map(Product::getName).collect(Collectors.toList());
    ...
    public List<String> categoriesToProductNames(Map<Category, List<Product>> productsByCategory, Stream<Category> categories) {
        ...
        return categories.flatMap(category -> productsByCategory.get(category).stream()).map(Product::getName).collect(Collectors.toList());
    }
}
