...
public class FunctionalInterfacesExercise02 {
    ...
    public Predicate<Product> createSearchPredicate(SearchCriteria criteria) {
        Predicate<Product> categoryIs = product -> product.getCategory().name().equals(criteria.category.name()); /* TODO: Implement a lambda expression that checks if a product's category is equal to criteria.category */;
